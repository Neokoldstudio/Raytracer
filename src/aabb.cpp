...
std::vector<double3> retrieve_corners(AABB aabb) {
    ...
    //S'assurer d'un volume = si rectangle est planaire bah ya aucun volume
     */
    ...
    //Un obj AABB est basically constitué d'un min et d'un max

    //On fait une liste pour storer les points
    std::vector<double3> coins;
    //On veut mettre les points uns par dessus les autres, pour s'assurer qu'ils restent allignés sur les axes


    //So basically, si on se met exactement au dessus du point minimum, mais à la hauteur du point maximum, ca nous donne le coin droit,devant,haut
    ...
    //FAUT faire des cas spéciaux:
    //Si |min.x| > |max.x| alors, on
    ...
    //On store les coins, (ya probablement une facon plus facile)
...
AABB construct_aabb(std::vector<double3> points) {
    ...
    */
    ...
    //FAUT RAJOUTER "<< std::endl" pour finir les impressions
    /*
